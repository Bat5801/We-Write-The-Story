class StoryGame {
    ...
    renderLevelList() {
        ...
        if (independentStories.length > 0) {
            ...
            independentStories.forEach(story => {
                ...
                const isLocked = story.locked || false;
                ...
                const clickAction = isLocked ? `onclick="game.showLockedAlert()"` : `onclick="game.startStory('${story.id}')"`;

                html += `
                    <div class="level-item ${isLocked ? 'locked' : ''}" ${clickAction}>
                        ...
                        <div class="level-status ${isCompleted ? 'completed' : 'incomplete'}">
                            ${deleteBtn}
                            ${isLocked ? '🔒 维护中' : (isCompleted ? '已完成' : '未完成')}
                        </div>
                    </div>
                `;
            });
    ...
    startCollection(collectionId) {
        ...
        collection.stories.forEach((story, index) => {
            ...
            html += `
                <div class="collection-story-item ${isLocked ? 'locked' : ''}" ${clickAction}>
                    <div class="story-number">${index + 1}</div>
                    <div class="story-info">
                        <h4>${story.title}</h4>
                        <div class="story-status">${isLocked ? '🔒 维护中' : (isCompleted ? '✓ 已完成' : '○ 未开始')}</div>
                    </div>
                </div>
            `;
        });
    ...
    showQTE(qteConfig) {
        ...
        if (qteConfig.type === 'click') {
            ...
            // 支持 sequence 和 keys 两种参数名（向后兼容）
            const sequence = qteConfig.sequence || qteConfig.keys || ['A', 'B', 'X', 'Y'];
            let currentIndex = 0;
            ...
            if (this.isMobile) {
                ...
                keyPanel.className = 'qte-key-panel';

                const uniqueKeys = [...new Set(sequence)];
                uniqueKeys.forEach(key => {
                    const keyButton = document.createElement('button');
                    keyButton.className = 'qte-key-button';
                    keyButton.textContent = key;
                    keyButton.addEventListener('click', () => handleInput(key));
                    keyPanel.appendChild(keyButton);
                });

                qteContainer.appendChild(keyPanel);
            }
    ...
    handleSkip() {
        if (this.isTyping) {
            this.stopTyping();
            const processedText = this.replaceVariables(this.currentNode.text);
            document.getElementById('story-text').textContent = processedText;

            if (this.currentNode.isEnd) {
                this.showEnding();
            } else if (this.currentNode.qte) {
                this.showQTE(this.currentNode.qte);
            } else if (this.currentNode.input) {
                this.showInput(this.currentNode.input);
            } else {
                this.showChoices();
            }
        }
    ...
    showEnding() {
        ...
        this.submitScoreToLeaderboard(endType);
    ...
    submitScoreToLeaderboard(endType) {
        if (!pgApi || !pgApi.currentUser) return;

        const score = this.calculateScore(endType);
        const playTime = this.getPlayTime();

        pgApi.submitScore(this.currentStory.id, score, playTime, endType).catch(e => {
    ...
    calculateScore(endType) {
        let baseScore = 1000;
        if (endType === 'death') return 0;
        if (endType === 'bad') baseScore = 500;
        if (endType === 'true') baseScore = 2000;

        const survivalBonus = (this.variables.health || 100) * 10;
        const itemBonus = this.inventory.length * 50;

        return baseScore + survivalBonus + itemBonus;
    }

    getPlayTime() {
        return Math.floor((Date.now() - (this.gameStartTime || Date.now())) / 1000);
    }

    showGoodEnding() {
        const message = document.getElementById('ending-message');
        const endMsg = this.currentNode.endMessage || '🎉 恭喜你完成了这个故事！';
        message.textContent = this.replaceVariables(endMsg);
        this.showScreen('victory-screen');
    }

    showBadEnding() {
        const message = document.getElementById('ending-message');
        const endMsg = this.currentNode.endMessage || '故事结束了，但这并不是最好的结局...';
        message.textContent = this.replaceVariables(endMsg);
        this.showScreen('victory-screen');
    }
    ...
    showTrueEnding() {
        const message = document.getElementById('ending-message');
        const endMsg = this.currentNode.endMessage || '✨ 你找到了真正的结局！这是最完美的故事！';
        message.textContent = this.replaceVariables(endMsg);
        this.showScreen('victory-screen');
    }
    ...
    showDeathScreen() {
        ...
        const endMsg = this.currentNode.endMessage || this.currentNode.text;
        message.textContent = this.replaceVariables(endMsg);
    ...
    updateSettingsUI() {
        ...
        if (typewriterToggle) {
            typewriterToggle.parentElement.classList.toggle('active', this.settings.typewriter);
        }
    }

    showLockedAlert() {
        this.showModal('🔒 故事维护中', '故事维护中，请耐心等待！', [
            { text: '我知道了', action: () => this.hideModal() }
        ]);
    }
...
function toggleSetting(settingName) {
    game.settings[settingName] = !game.settings[settingName];
    game.saveSettings();
    game.updateSettingsUI();
}
...
function checkLoginStatus() {
    ...
    if (user) {
        ...
        userStatus.innerHTML = '<button class="logout-btn" onclick="showLogin()">登录</button>';
        userStatus.classList.add('logged-in');
    }
}
...
async function handleLogin() {
    ...
    if (!username || !password) {
        document.getElementById('login-error').textContent = '请填写用户名和密码';
        return;
    }

    try {
        await pgApi.login(username, password);
        closeLogin();
        checkLoginStatus();
        game.showAlert('登录成功！', '🎉 欢迎回来');
    } catch (e) {
...
async function handleRegister() {
    ...
    if (!username || !password) {
        document.getElementById('register-error').textContent = '请填写用户名和密码';
        return;
    }

    if (username.length < 3) {
        document.getElementById('register-error').textContent = '用户名至少3个字符';
        return;
    }

    if (password.length < 4) {
        document.getElementById('register-error').textContent = '密码至少4个字符';
        return;
    }

    try {
        await pgApi.register(username, password, nickname);
        closeRegister();
        showLogin();
        game.showAlert('注册成功！请登录', '🎉 注册成功');
    } catch (e) {
...
async function loadLeaderboard() {
    ...
    try {
        const data = await pgApi.getLeaderboard('', 50);
            ...
            const itemHtml = `
                <div class="leaderboard-item ${rankClass}">
                    ...
                    <div class="leaderboard-info">
                        ...
                        <p>${item.storyId || '未知故事'}</p>
                    </div>
                    <div class="leaderboard-score">${parseInt(item.score) || 0}</div>
                </div>
            `;
        ...
        loading.style.display = 'none';
        list.innerHTML = `<div class="leaderboard-empty">加载失败: ${e.message}</div>`;
    }
}