class StoryGame {
    ...
    loadImportedStories() {
        try {
            ...
            stories.forEach(story => {
                ...
                const existingStory = this.stories.find(s => s.id === story.id);
                if (!existingStory) {
            ...
            console.log('[LocalStorage] 加载导入故事失败:', err);
            return [];
        }
    }
    ...
    saveImportedStory(story) {
        try {
            ...
            console.log('[LocalStorage] 保存导入故事:', story.title);
        } catch (err) {
            console.log('[LocalStorage] 保存失败:', err);
        }
    }
    ...
    clearImportedStories() {
        ...
        this.stories = this.stories.filter(s => {
            // 保留内置故事（从 fallbackStories 或 storyFiles 加载的）
            const builtInIds = new Set();
            if (window.fallbackStories) {
                window.fallbackStories.forEach(s => builtInIds.add(s.id || s.title.replace(/[^a-zA-Z0-9]/g, '_')));
            }
    ...
    async loadDefaultStories() {
        if (window.fallbackStories) {
            window.fallbackStories.forEach(story => {
                if (!story.id) {
                    story.id = story.title.replace(/[^a-zA-Z0-9]/g, '_');
                }
                ...
                if (!story.start && story.nodes.start) {
                    story.start = 'start';
                }

                this.stories.push(story);
            });
            console.log('[StoryLoader] 成功加载', this.stories.length, '个故事');
        } else {
            console.error('[StoryLoader] 未找到故事数据');
        }
        ...
        // 强行调用 DLC: 盗棺笔记系列（直接从 StoryDLC 目录加载，不经过 stories.js）
            ...
            xhr.onload = () => {
                if (xhr.status === 200) {
                    try {
                        ...
                        if (this.validateStory(storyData)) {
                            this.processStoryData(storyData, fileName);
    ...
    processStoryData(storyData, fileName) {
        if (this.validateStory(storyData)) {
            const storyId = fileName.replace('.story', '');
    ...
    startStory(storyId) {
        ...
        if (story.features && story.features.inventory && story.items) {
            this.inventory = [...story.items];
        }
    ...
    updateInventoryDisplay() {
        ...
        this.inventory.forEach(item => {
            ...
            if (!icon) {
                ...
                else if (item.name.includes('锤子') || item.includes('匕首')) icon = '🔨';
    ...
    showChoices() {
        ...
        node.choices.forEach((choice, index) => {
            if (choice.condition) {
                if (choice.condition.hasItem && !this.hasItem(choice.condition.hasItem)) {
                    return;
                }
                if (choice.condition.notHasItem && this.hasItem(choice.condition.notHasItem)) {
                    return;
                }
                if (choice.condition.flag && !this.gameState[choice.condition.flag]) {
                    return;
                }
            }
